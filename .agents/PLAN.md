...
纯 native 基准保持不变；解释器密集的标量循环、global 算术、容器和闭包均有可重复收益，因此保留该结构调整。标准库单项的 2.8–3.5% 小幅回退在两次测量中重复出现，后续内部指令优化必须继续跟踪，不能直接归为噪声。阶段 2 下一步优先改造内部指令表示，再评估静态 global slot 和可证明安全的 frame 复用。
...
- 将通用 `OpCode + Vec<i64>` 指令逐步替换为固定字段的内部指令，消除参数堆分配、索引和整数转换。
- 为静态 global 建立每次执行的解析 slot；context 中用户值覆盖同名标准库值的规则保持不变。
- 对不产生闭包捕获的循环复用 frame/register 存储；可能被闭包捕获的迭代仍建立独立词法环境。
