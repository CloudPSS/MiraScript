...
阶段 3 的静态小参数调用 quickening 已完成。无 spread 的静态调用在解码时改写为 `CallGlobal0..4`，执行时直接借用当前 run 已解析的 context slot，不再克隆调用目标或经过通用参数长度分派。slot 仍在每次 run 重新解析，因此更换 context 或用户覆盖 builtin 会自然失效。

两次完整 benchmark 与阶段 2 最终区间比较，native call 从 152.1–157.1 ns 降至 141.9–143.6 ns，改善 5.6–9.7%；简单脚本 `run_with` 从 358.2–379.2 ns 降至 320.0–322.8 ns，改善 9.9–15.6%。标量循环和容器也分别改善约 4–7% 与 2–5%，其他项目未发现稳定回退，因此保留该 quickening。
...
- 继续评估数值算术和常见数学 builtin 的专用指令；不得绕过统一参数验证、超时和错误模型。
