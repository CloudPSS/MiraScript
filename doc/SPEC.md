...
常量模式用于匹配常量的值。常量模式的语法为 `<constant>`，其中 `<constant>` 是 `nil`、`true`、`false`、`nan`、`inf`、数字字面量或不包含插值的字符串字面量，及它们与前缀运算符 `+`、`-` 结合的结果。

与 `==` 运算符不同，常量模式使用相同值语义进行匹配。
...
关系模式用于匹配关系运算的结果。关系模式的语法为 `<relation> <constant>`，其中 `<relation>` 是 `>`、`<`、`<=`、`==`、`!=`、`~=`、`!~=` 运算符，`<constant>` 是一个常量模式。

关系模式相当于对匹配到的值进行 `<captured> <relation> <constant>` 的判断，当该判断返回 `false` 或抛出异常时，匹配失败。
...
范围模式用于匹配数字或字符串范围。范围模式的语法为 `<start>..<end>` 或 `<start>..<<end>`，其中 `<start>` 和 `<end>` 是数字或字符串的常量模式。
