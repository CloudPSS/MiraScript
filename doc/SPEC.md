...
除了 `extern` 和 `function` 外，MiraScript 的所有类型均为值类型。这意味着在进行比较时，MiraScript 会比较数据类型的值，而不是它们的引用。

除了 `extern` 类型，MiraScript 的所有类型都是不可变的。不可变性意味着一旦创建，数据的值就不能被修改。
...
标识符以 `$`、`@`、`_` 或字母开头，后面可以跟任意数量的字母、数字、`$`、`@` 或 `_`。标识符不能包含空格和其他特殊字符。

标识符是区分大小写的，`a` 和 `A` 是不同的标识符。

以 `__` 开头或包含 `$`、`@` 的标识符保留供 MiraScript 或全局环境使用，用户不应使用这些标识符。
...
println(named_record.name); // 输出 "Alice"
println(named_record["age"]); // 输出 "30"

let unnamed_record = (-4, 3);
println(`${unnamed_record.0}, ${unnamed_record[1]}`); // 输出 "-4, 3"
```
...
```rust
let array = [1, 2, 3, 4, 5];
println(array[1..3]);   // 输出 "2, 3, 4"
println(array[1..<3]);  // 输出 "2, 3"
println(array[1..]);    // 输出 "2, 3, 4, 5"
println(array[..3]);    // 输出 "1, 2, 3, 4"
println(array[..<3]);   // 输出 "1, 2, 3"
println(array[..]);     // 输出 "1, 2, 3, 4, 5"
println(array[1..-2]);  // 输出 "2, 3, 4"
println(array[4..<-2]); // 输出 ""
```

### 范围

范围表示一个值的范围。范围的语法为 `start..end` 或 `start..<end`，其中 `start` 和 `end` 是一个表达式，表示范围的起始值和结束值。
...
|        |          |          | 非空断言 `x!`        | 当 `x` 为 `nil` 时，抛出 `NilError` 异常     |
|        |          |          | `type(x)`            | 结果为 `x` 类型的字符串                      |
...
|        | 关系运算 | 从左到右 | 包含 `x in y`        | 测试记录是否包含某个键，或数组是否包含某个值 |
...
|        | 逻辑或   | 从左到右 | 逻辑或 `x \|\| y`    |                                              |
|        | 空合并   | 从左到右 | 空合并 `x ?? y`      |                                              |
| 最低   | 管道     | 从左到右 | 正向管道 `x \|> y`   | `y` 为一个函数或调用表达式                   |
|        |          |          | 反向管道 `x <\| y`   | `x` 为一个函数或调用表达式                   |
...
MiraScript 的成员访问操作符 `.` 和 `[]` 也支持短路求值，当访问链的某处返回 `nil` 时，后续的成员访问操作符不会被求值。

```rust
let x = (1);

let index = 2;
let y = x.1[index]; // y 的值为 nil，index 不会被求值

let args = [1, 2];
let z = x.1(args || []); // z 的值为 nil，args || [] 不会被求值
```

使用 `()` 操作符可以强制求值，即使左操作数为 `nil`。

```rust
let x = (1);

let index = 2;
let y = (x.1)[index]; // y 的值为 nil，index 被求值

let args = [1, 2];
let z = (x.1)(args || []); // args || [] 被求值，但 (x.1) 不可被调用，抛出 TypeError 异常
```
...
x >= w; // 尝试将 w 转换为 number 类型，抛出 TypeError 异常
z < w; // z 转换为 string 类型 ""，尝试将 w 转换为 string 类型，抛出 TypeError 异常
```
...
管道运算符 `|>` 和 `<|` 将一个值传递给一个函数或调用表达式，用以简化函数调用的语法。

对于正向管道 `|>`，当右操作数为一个调用表达式时，左操作数会被插入到右操作数的第一个参数；当右操作数为一个函数时，左操作数会作为唯一参数调用该函数。

对于反向管道 `<|`，当左操作数为一个调用表达式时，右操作数会被插入到左操作数的最后一个参数；当左操作数为一个函数时，右操作数会作为唯一参数调用该函数。

```rust
[1, 2, 3] |> filter(fn { it > 1 }) |> println;
// 相当于 println(filter([1, 2, 3], fn { it > 1 }));
println <| ([1, 2, 3] |> filter(fn { it > 1 }));
// 注意管道运算符是左结合的，此处需要使用括号
```
...
字面量模式用于匹配字面量的值。字面量模式的语法为 `<literal>`，其中 `<literal>` 是 `nil` `true` `false`、数字字面量或不包含插值的字符串字面量。
...
关系模式用于匹配关系运算的结果。关系模式的语法为 `<relation> <literal>`，其中 `<relation>` 是 `>` `<` `>=` `<=` `==` `!=` `~=` `!~=` 运算符，`<literal>` 是一个字面量模式。

关系模式相当于对匹配到的值进行 `<captured> <relation> <literal>` 的判断，当该判断返回 `false` 或抛出异常时，匹配失败。
...
范围模式用于匹配数字或字符串范围。范围模式的语法为 `<start>..<end>` 或 `<start>..<<end>`，其中 `<start>` 和 `<end>` 是数字或字符串的字面量模式。
...
- 具名模式
  ...
  可以使用 `?:` 语法表示该模式是可选的，此时对不存在的键会匹配到 `nil`。
...
- 元素模式
  ...
  ```rust
  let array = [1, 2, 3];
  array is [1, x];       // 匹配成功，x 的值为 2
  array is [x, y, z, w]; // 匹配失败，元素数量不足
  array is [(1..10), 2]; // 匹配成功，范围模式须添加括号
  ```
