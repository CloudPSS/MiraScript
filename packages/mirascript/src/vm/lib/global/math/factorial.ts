...
const TAU = 2 * Math.PI;
const SQRT_TAU = sqrt(TAU);

/** 计算非整数的阶乘 */
function floatFactorial(n: number): number {
    if (n > 85) {
        ...
        return SQRT_TAU * pow(t, n + 0.5) * exp(-t) * p;
    }
}

const CACHE: Array<number | undefined> = [1, 1];
/** 计算整数的阶乘 */
function intFactorial(n: number): number {
