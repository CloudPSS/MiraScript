...
[![Visual Studio Marketplace](https://img.shields.io/visual-studio-marketplace/v/CloudPSS.mirascript?label=VS%20Code%20Marketplace&cacheSeconds=14400)](https://marketplace.visualstudio.com/items?itemName=CloudPSS.mirascript)
[![Open VSX Registry](https://img.shields.io/open-vsx/v/CloudPSS/mirascript?label=Open%20VSX%20Registry&cacheSeconds=14400)](https://open-vsx.org/extension/CloudPSS/mirascript)
...
本扩展支持通过 [cosmiconfig](https://github.com/cosmiconfig/cosmiconfig) 规范加载项目配置文件，配置名称为 `mira`。支持的配置文件包括：

- `package.json` 中的 `"mira"` 字段
- `.mirarc`、`.mirarc.json`、`.mirarc.yaml`、`.mirarc.yml`
- `.mirarc.js`、`.mirarc.cjs`、`.mirarc.mjs`、`.mirarc.ts`
- `mira.config.js`、`mira.config.cjs`、`mira.config.mjs`、`mira.config.ts`
