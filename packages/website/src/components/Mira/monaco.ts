...
`,
        `file:///mirascript-operation.d.ts`,
    );
    ...
    monaco.languages.registerCodeLensProvider(
        ['mirascript', 'mirascript-template', 'mirascript-doc'].map((language) => ({ language, scheme: 'markdown' })),
        {
            provideCodeLenses(model, token) {
                const { uri } = model;
                const query = uri.query ? new URLSearchParams(uri.query) : null;
