...
| ^        | ^        | 模板调用 `x"y"`      | `"y"` 是一个插值字符串             |
| ^        | ^        | 非空断言 `x!`        |                                    |
| 乘方运算 | 从右到左 | 乘方 `x^y`           | 结果为 `number` 类型               |
| 前缀操作 | /        | 逻辑非 `!x`          | 结果为 `boolean` 类型              |
| ^        | ^        | 一元加 `+x`          | 结果为 `number` 类型               |
| ^        | ^        | 一元减 `-x`          | ^                                  |
| 乘法运算 | 从左到右 | 乘法 `x * y`         | 结果为 `number` 类型               |
| ^        | ^        | 除法 `x / y`         | ^                                  |
| ^        | ^        | 取余 `x % y`         | ^                                  |
| 加法运算 | 从左到右 | 加法 `x + y`         | 结果为 `number` 类型               |
| ^        | ^        | 减法 `x - y`         | ^                                  |
| 模式匹配 | 从左到右 | 模式匹配 `x is y`    | `x` 是一个表达式，`y` 是一个模式   |
| 关系运算 | 从左到右 | 包含 `x in y`        | `y` 是一个复合类型或 `global`      |
| ^        | ^        | 不包含 `x not in y`  | 等价于 `!(x in y)`                 |
...
| 逻辑或   | 从左到右 | 逻辑或 `x \|\| y`    | ^                                  |
| 空合并   | 从左到右 | 空合并 `x ?? y`      |                                    |
| 条件运算 | 从右到左 | 条件运算 `c ? x : y` | 条件为 `boolean` 类型              |
...
使用 `in` / `not in` 操作符测试记录是否包含某个键，或数组是否包含某个值，或全局变量是否存在。

```mira
let x = (nil, );
let y = 0 in x;       // y 的值为 true (0 会先被转换为字符串);
let z = 1 in x;       // z 的值为 false;
let w = '0' not in x; // w 的值为 false;

let a = ["hello", "world"];
let b = "hello" in a; // b 的值为 true;

let has_sin = 'sin' in global; // has_sin 的值为 true;
```
