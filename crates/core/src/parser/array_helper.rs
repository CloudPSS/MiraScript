...
type _ArrayElement<'s, 'a, E, S> = ListItem<'s, 'a, ArrayElementBase<'s, 'a, E, S>>;
fn array_element<'s: 'a, 'a, E: PartialEq + 'a, S: PartialEq + 'a>(
    arena: &'a AstArena,
    element: impl Parser<'s, E>,
    spread: impl Parser<'s, S>,
    mut missing: impl FnMut(usize) -> E + Copy,
) -> impl Parser<'s, _ArrayElement<'s, 'a, E, S>> {
    move |i: &mut Input<'s>| {
        ...
        if *first == Operator::Comma {
            ...
            return Ok(ListItem::new_with_comma(
                arena,
                ArrayElementBase::Element(arena.alloc(missing)),
                comma,
            ));
        }
        ...
        let result = if *first == Operator::SpreadRange {
            (token(Operator::SpreadRange), spread)
                .map(|(s, e)| ArrayElementBase::Spread(s, arena.alloc(e)))
                .parse_next(i)?
        } else {
            element
                .map(|e| ArrayElementBase::Element(arena.alloc(e)))
                .parse_next(i)?
        };
...
type _ArrayLike<'s, 'a, E, S> = (TokenRef<'s>, Vec<_ArrayElement<'s, 'a, E, S>>, TokenRef<'s>);
pub(super) fn array_base<'t, 's: 't + 'a, 'a, E: PartialEq + 'a, S: PartialEq + 'a>(
    arena: &'a AstArena,
...
) -> impl Parser<'s, _ArrayLike<'s, 'a, E, S>> {
    move |i: &mut Input<'s>| {
        let open = open.parse_next(i)?;
        let parts: Vec<_> =
            repeat(0.., array_element(arena, element, spread, missing)).parse_next(i)?;
